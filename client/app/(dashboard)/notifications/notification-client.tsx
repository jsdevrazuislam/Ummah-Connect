          ...
          {notifications.map((notification) => {
            ...
            return (
              <div key={notification?.id} className="p-4 relative cursor-pointer border-b border-border hover:bg-muted/50">
                {
                  !notification?.isRead && <div className="w-2 h-2 rounded-full bg-primary absolute right-7 top-[60%]" />
                }
                <div className="flex gap-3">
                  <div className="mt-1 relative">
                    <Avatar className="h-14 w-14">
                      {notification?.sender?.avatar
                        ? <AvatarImage src={notification?.sender?.avatar} alt={notification?.sender?.fullName} />
                        : <AvatarFallback>{notification?.sender?.fullName?.charAt(0)}</AvatarFallback>}
                    </Avatar>
