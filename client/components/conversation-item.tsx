...
import { Archive, Flag, MoreHorizontal, Trash2, User } from "lucide-react";
...
import { deleteConversation } from "@/lib/apis/conversation";
...
const ConversationItem: FC<ConversationItemProps> = ({ conv, onClick }) => {
  ...
  const encryptedSymmetricKey = user?.id === conv?.lastMessage?.sender?.id
    ? conv?.lastMessage?.keyForSender
    : conv?.lastMessage?.keyForRecipient;
        ...
        <div className="flex gap-3 items-center">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs text-muted-foreground">{formatDistanceToNow(new Date(conv?.lastMessage?.sentAt ?? ""))}</span>
            </div>
            <div className="flex justify-between items-center">
