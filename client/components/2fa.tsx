...
export function TwoFactorAuth() {
  const { user } = useStore();
  const [is2FAActive, setIs2FAActive] = useState(user?.isTwoFactorEnabled);
  ...
  const { mutate: muFun, isPending: isLoading } = useMutation({
    ...
    onSuccess: (data) => {
      showSuccess("2FA verified and enabled successfully");
      setIs2FAActive(true);
      setShowVerificationSetup(false);
  ...
  return (
    showBackupCode
      ? <BackupCodes backUpCodes={backUpCodes} setShowBackupCode={setShowBackupCode} />
      : (
          <Card>
            <CardHeader>
