...
export function isSpam(userId: string, content: string): boolean {
  ...
  if (user.messages.length > 5) user.messages.shift(); 
  ...
  if (user.messages.every(msg => msg === content)) return true;

  const bannedWords = ["buy now", "subscribe", "click here", "visit my channel"];
  if (bannedWords.some(w => content.toLowerCase().includes(w))) return true;
